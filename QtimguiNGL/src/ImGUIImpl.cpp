...
void ImGui_ImplQt_CreateFontsTexture()
{
    // Build texture atlas
    ImGuiIO& io = ImGui::GetIO();
...
{
  std::cout<<"Create Device Objects IMGUI\n";
    // Backup GL state
...
#undef OFFSETOF
    ...
    ImGui_ImplQt_CreateFontsTexture();
...
void ImGui_ImplQt_NewFrame(NGLScene *window)
{
    if (!g_FontTexture)
    ...
    int display_w=window->width();
    int display_h=window->height();
    io.DisplaySize = ImVec2(display_w,display_h);
    float aspect=window->devicePixelRatio();
    io.DisplayFramebufferScale = ImVec2(aspect,aspect);
